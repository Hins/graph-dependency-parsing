...
with open('lang_zh/embeddings/vectors-labels.txt', 'r') as file:
    for line in file.readlines():
        label = line.split(' ')[0].lower()
        i2label[label2i[label]] = label

with open('lang_zh/embeddings/label2i.pickle', 'wb') as file:
    pickle.dump(dict(label2i), file)

with open('lang_zh/embeddings/i2label.pickle', 'wb') as file:
